...
export default function HotelDetail() {
    ...
    const handleStateModalListImagesChange = (state: boolean, idImageCurrent: number) => {
        setModalListImagesState(state);
        setCurrentImageModal(idImageCurrent);
    }
    const getListTypeImage = (): TypeImage[] => {
        var listTypes: TypeImage[] = [];
        hotel?.images && hotel?.images.map((item) => {
            var listType = { TypeName: item.TypeRoom?.split(';')[1], Total: 0, FirstImage: '' };
            hotel.images.map((jItem) => {
                if (jItem.TypeRoom?.split(';')[1] === listType.TypeName) {
                    listType.Total += 1;
                    if (listType.FirstImage === '') { listType.FirstImage = jItem.FileName }
                }
            });
            ...
            {hotel !== undefined ?
             ...
             my-10 rounded-lg'>
                    ...
                    <div className='w-full lg:w-10/12 flex flex-col lg:flex-row flex-wrap bg-slate-100 rounded-lg'>
                        ...
                        <div className='w-full lg:w-4/12 p-3'>
                            <p className='text-xl text-gray-900'><b>Địa điểm lân cận</b></p>
                            <ul>
                        ...
                        <div className='w-full lg:w-4/12 p-3'>
                            <p className='text-xl text-gray-900'><b>Phổ biến trong khu vực</b></p>
                            <ul>
                                {diadiemlancan.filter((item) => {
                        ...
                        <div className='w-8/12 pb-2 flex flex-row'>
                            ...
                            <span>Khoảng cách hiển thị dựa trên đường chim bay. Khoảng cách di chuyển thực tế có thể khác.</span>
                        </div>
                    </div>
                    ...
                    <div className='my-3 w-full lg:w-10/12 flex flex-col lg:flex-row flex-wrap
                     bg-slate-100 rounded-lg'>
                        ...
                        <div className={`flex p-3 flex-col ${showDescript ? 'h-auto' : 'h-[180px] overflow-hidden'}`}>
                            <p className='text-2xl text-gray-900'><b>Vị trí</b></p>
                            <p className='text-lg'>{hotel.LocationDetail}</p>
                            <p className='text-2xl text-gray-900'><b>Thông tin về {hotel.Name}</b></p>
                            <p className='text-lg'>{hotel.Description}</p>
                        </div>
                        <button className='text-lg font-bold text-cyan-500
                        border border-1 border-cyan-500 round-sm px-1 flex flex-row m-3'
                            ...
                            {showDescript ?
                                <span><svg className="w-6 h-6 text-cyan-500 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                     ...
                     bg-slate-100 rounded-lg p-3'>
                        ...
                        <div className='flex flex-row p-2 w-full lg:w-8/12'>
                            ...
                            <div className='w-full'>
                                <p className='text-2xl'><b>Thời gian nhận phòng/trả phòng</b></p>
                                <div className='flex flex-row w-full'>
                                    <p className='text-xl text-gray-900 font-semibold'>Giờ nhận phòng:
                                        <b> Từ {hotel.TimeCheckIn}</b></p>
                                    <p className='text-xl text-gray-900 font-semibold ml-5'>Giờ trả phòng:
                                        <b> Trước  {hotel.TimeCheckOut}</b></p>
                                </div>
                                <hr />
                                {hotel.policies && hotel.policies.length > 0 ?
                                    ...
                                    : <p className='text-gray-900 text-lg'>Chính sách của khách sạn không có sẵn</p>}
                                ...
                                <div className='flex flex-rowpy-1 font-semibold overflow-hidden py-1'>
                                    ...
                                    <p className='w-8/12'>	Từ {hotel.TimeCheckIn} - Trước {hotel.TimeCheckOut}</p>
                                </div>
